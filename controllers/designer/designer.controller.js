...
} = require("../../controllerMessages/designerTask/designerTask.messages");
...
const addDesigner = async (req, res) => {
  try {
    ...
    let currentMinutes = String(currentTimeDetails.getMinutes() + 1).padStart(
      2,
      "0"
    );
    let currentSeconds = currentTimeDetails.getSeconds();
    let result = "";
    let currentHoursMinutesSeconds = result.concat(
      currenthours,
      ":",
      currentMinutes,
      ":",
    ...
    if (
      parseInt(givenStartTime.replace(regex, ""), 10) >=
      parseInt(currentHoursMinutesSeconds.replace(regex, ""), 10)
    ) {
      ...
      parseInt(givenEndTime.replace(regex, ""), 10) <=
      parseInt(currentHoursMinutesSeconds.replace(regex, ""), 10)
    ) {
      const responsePayload = {
        status: 0,
            ...
            if (hours <= 4) {
              ...
              if (designerData1.length > 0) {
                ...
                if (hours <= remainingHours) {
                  ...
                  let designerData = await insertData(
                    team,
                    project,
                    hours,
                    newDate,
...
const listAllDesignerTask = async (req, res) => {
  try {
    ...
    let dateDetails = [];
    let allDesignerTask = await Designer.find({
          ...
          if (lastSaturday === 4 || lastSaturday === 5) {
            if (hours <= 4) {
              ...
              if (designerData1.length > 0) {
                ...
                if (hours <= remainingHours) {
                  let newDate = new Date(date).toISOString().split("T")[0];
                  let updateDesignerTask = await Designer.findByIdAndUpdate(
                    id,
                    {
                      team,
                      project,
